...
ELK_SERVICES   := es01 es02 es03 kibana logstash filebeat packetbeat metricbeat heartbeat
...
swarm-deploy-elk:
	@make build
	docker stack deploy -c docker-compose.yml netelk

swarm-deploy-monitoring:
	@make build
	@docker stack deploy -c docker-compose.yml -c docker-compose.monitor.yml netelk

swarm-deploy-tools:
	@make build
	@docker stack deploy -c docker-compose.yml -c docker-compose.tools.yml netelk

swarm-rm:
	docker stack rm netelk
